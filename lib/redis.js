...
function poiCache(_id, callback) {
    callback = callback || callbackFn;

    if (_id > 0) {
        console.log('poiCache: ' + _id);
        then(function (defer) {
            client[globalCacheDb].hgetall('poi.hash.' + _id, defer);
        }).then(function (defer, poi) {
            if (!poi) {
                callback(jsGen.Err('redis poi not exist!'));
            } else {
                poi._id = +poi._id;
                poi.articles = +poi.articles;
                poi.users = +poi.users;
                callback(null, poi);
            }
        }).fail(function (defer, err) {
        ...
        callback(jsGen.Err('redis poiCache request error!'));
    }
}
poiCache.index = function (start, end, callback) {
    start = start || 0;
    end = end > 1 ? end - 1 : -1;
    callback = callback || callbackFn;
    then(function (defer) {
        client[globalCacheDb].zrevrange('poi.sset.index', start, end, defer);
    }).then(function (defer, list) {
        callback(null, toNum(list || []));
    }).fail(function (defer, err) {
        callback(err);
...
poiCache.index.total = function (callback) {
    callback = callback || callbackFn;
    then(function (defer) {
        client[globalCacheDb].zcard('poi.sset.index', defer);
    }).then(function (defer, total) {
        callback(null, +total);
    }).fail(function (defer, err) {
        callback(err);
...
poiCache.get = function (name, callback) {
    callback = callback || callbackFn;

    if (name) {
        then(function (defer) {
            client[globalCacheDb].get(('poi.str.' + name).toLowerCase(), defer);
        }).then(function (defer, _id) {
            callback(_id > 0 ? null : jsGen.Err('redis poi not exist!'), +_id);
        }).fail(function (defer, err) {
            callback(err);
        });
    } else {
        callback(jsGen.Err('redis poiCache.get request error!'));
    }
};
poiCache.update = function (poiObj, callback) {
    var poi = {
        _id: 0,
        poi: 0,
        articles: 0,
    ...
    console.log('poiCache.update:' + poi.poi);
    callback = callback || callbackFn;
    intersect(poi, poiObj);
    if (poi._id > 0) {
        then(function (defer) {
            client[globalCacheDb].hgetall('poi.hash.' + poi._id, defer);
        }).all(function (defer, err, old) {
            if (old && poi.poi && old.poi !== poi.poi) {
                client[globalCacheDb].del(('poi.str.' + old.poi).toLowerCase(), defer);
            } else {
                defer();
            }
        }).then(function (defer) {
            client[globalCacheDb].hmset('poi.hash.' + poi._id, poi, defer);
        }).then(function (defer) {
            client[globalCacheDb].zadd('poi.sset.index', poi.articles, poi._id, defer);
        }).then(function (defer) {
            client[globalCacheDb].set(('poi.str.' + poi.poi).toLowerCase(), poi._id, defer);
        }).then(function (defer) {
            callback(null, true);
        }).fail(function (defer, err) {
            callback(err);
        ...
        callback(jsGen.Err('redis poiCache update request error!'));
    }
};
poiCache.remove = function (_id, callback) {
    callback = callback || callbackFn;
    then(function (defer) {
        client[globalCacheDb].del('poi.hash.' + _id, defer);
    }).then(function (defer) {
        client[globalCacheDb].zrem('poi.sset.index', _id, defer);
    }).then(function (defer) {
        callback(null, true);
    }).fail(function (defer, err) {
        callback(err);
...
poiCache.removeAll = function (callback) {
    callback = callback || callbackFn;
    then(function (defer) {
        client[globalCacheDb].keys('poi*', defer);
    }).then(function (defer, keys) {
        if (keys.length > 0) {
            client[globalCacheDb].del(keys, defer);
        } else {
